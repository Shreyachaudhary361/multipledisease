...
parkinson_dataset = pd.read_csv('parkinsondata.csv')


with st.sidebar:
    selected = option_menu('Mulitple Disease Prediciton System',
                           ['Diabetes Prediction',
...
if selected == 'Diabetes Prediction':
    ...
    if st.button('Diabetes Test Result'):
        ...
        std_data = [[Pregnancies, Glucose, BloodPressure, SkinThickness, Insulin, BMI, DiabetesPedigreeFunction, Age]]
...
if selected == 'Parkinson\'s Disease Prediction':
    ...
    if st.button("Parkinson's Test Result"):
        ...
        scaler = StandardScaler()
        scaler.fit_transform(X)
        std_data = [[fo, fhi, flo, Jitter_percent, Jitter_Abs, RAP, PPQ,DDP,Shimmer,Shimmer_dB,APQ3,APQ5,APQ,DDA,NHR,HNR,RPDE,DFA,spread1,spread2,D2,PPE]]
        std_data = scaler.transform(std_data)
        
        parkinsons_prediction = parkinson_model.predict(std_data)                          
